...
#if defined(NDEBUG)
# error "Haocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0f7c95eefe2a86c3098114b8ee9e5ff57f4c28970bd61a3cd0bc91997f239d53");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Haocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Haocoin Signed Message:\n";
...
{
    // Haocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Haocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Haocoin: 1 day(s)
static const int64 nTargetSpacing = 5 * 60; // Haocoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Haocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Haocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2701b42168400cc34faa12d6a82aab10e70ac18e6e5a1ee3624794948c71e64d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Indepence day of Vietnam is September 2rd 1945.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ae593ed21fad40768defe8a9cc8e22a78126dc06a54a5f80e2d0459e044a7bd032182e117553681c00f63e481a13aab5661622a89d4999d4f56f79358f0664cb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615883834;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084661533;

        if (fTestNet)
        {
            block.nTime    = 1615883829;
            block.nNonce   = 385999763;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc630d1c4a2228c497b03d88bdac6c4b2c253f46b678e4631396ecc172dc0103e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Haocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HaocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HaocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HaocoinMiner(CWallet *pwallet)
{
    printf("HaocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("haocoin-miner");
    ...
    try { loop {
        ...
        printf("Running HaocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HaocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HaocoinMiner, pwallet));
}
